...
// Student message to admin (optional multiple file attachments, max 3 files / 15MB combined)
const STUDENT_MESSAGE_MAX_TOTAL_BYTES = 15 * 1024 * 1024;
app.post('/api/messages/student-to-admin', upload.array('attachments', 3), async (req, res) => {
    try {
        ...
        if (totalBytes > STUDENT_MESSAGE_MAX_TOTAL_BYTES) {
            return res.status(400).json({ success: false, error: 'Combined attachment size cannot exceed 15MB' });
        }
...
// Reviewer message to admin (optional multiple file attachments, max 3 files / 15MB combined)
const REVIEWER_MESSAGE_MAX_TOTAL_BYTES = 15 * 1024 * 1024;
